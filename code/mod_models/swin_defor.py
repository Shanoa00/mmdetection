...
dataset_name= 'Nancho_dataset/'  #kuzushiji, #HanDataset,  #Nancho_dataset,
...
data = dict(
    ...
    train=dict(
        ...
        type=dataset_type,
        classes=classes,
        # ann_file=data_root + 'dtrain_crop2.pkl',  #any other dataset
        # img_prefix=data_root + 'train_crops2/',  # any
        ann_file=data_root + 'dtrain_crop1240.pkl',  #nancho
        img_prefix=data_root + '0train_crops1/',  #nancho
        pipeline=train_pipeline #)
        ),
    val=dict(
        type=dataset_type,
        classes=classes,
        # ann_file=data_root + 'dval.pkl', #any
        # img_prefix=data_root + 'train_images/', #any
        ann_file=data_root + '0dval.pkl', #nancho
        img_prefix=data_root + 'train_images/', #nancho
        pipeline=val_pipeline),
    test=dict(
        type=dataset_type,
        classes=classes,
        # ann_file=data_root + 'dtest.pkl',  #any
        # img_prefix=data_root + 'test_images/', #any
        ann_file=data_root + '0dtest.pkl',  #nancho
        img_prefix=data_root + 'test_images/', #nancho
...
lr_config = dict(
    ...
    # warmup='linear',
    # warmup_ratio=1.0 / 3,
    min_lr=3.294e-04,  # added
    step=[8, 17, 26, 35, 42],  #[8,14,20]
)
# We can set the checkpoint saving interval to reduce the storage cost
checkpoint_config = dict(interval=10)  #25
# yapf:disable
log_config = dict(
    interval=100, #50
    hooks=[
...
log_level = 'INFO'
work_dir = './work_dirs/' + dataset_name+'defV3/' #swin_kuzushiji, swin_nancho_deformable2, swin_nancho_deformable2/scales, S03/swin_t
